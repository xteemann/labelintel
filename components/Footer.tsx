      ...
      <div className="container-tight">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-10">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {['Features', 'Pricing', 'Sample Report', 'API Docs', 'Changelog'].map((link) => (
                <li key={link}>
                  <a href={`#${link.toLowerCase().replace(' ', '-')}`} className="text-sm text-zinc-400 hover:text-white transition-colors">
                    {link}
                  </a>
                </li>
              ))}
        ...
        <div className="flex flex-col md:flex-row items-center justify-between pt-8 border-t border-brand-border gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* LinkedIn */}
            <a href="#" className="text-brand-muted hover:text-white transition-colors" aria-label="LinkedIn">
              <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
